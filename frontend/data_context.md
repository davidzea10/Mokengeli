# Contexte des données — portail client (app `client/`)

Ce document décrit **comment les données sont préparées et envoyées** depuis l’app React **client** vers le backend pour le **simulateur client** (`http://localhost:5174` par défaut, Vite). Il sert de référence pour l’implémentation côté API et l’alignement avec `docs/transaction_schema.json`.
...
| Élément | Rôle |
|--------|------|
| **Code source** | `client/src/api/` (contrats, construction des payloads, HTTP) |
| **Page React** | `client/src/pages/Client.tsx` |
...
- Fichier d’exemple : `client/.env.example`

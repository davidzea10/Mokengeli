...
export function Dashboard({ activeTab: initialTab }: DashboardProps) {
  ...
  const detailTxFromUrl = useMemo(() => {
    ...
    const inRemote = remoteTransactions.find(
      (t) => t.transaction_event.metadata.numero_transaction === txFromUrl,
    );
    if (inRemote) return inRemote;
    return (
      transactionsListRows.find(
        (t) => t.transaction_event.metadata.numero_transaction === txFromUrl,
      ) ?? null
    );
  }, [txFromUrl, remoteTransactions, transactionsListRows]);
